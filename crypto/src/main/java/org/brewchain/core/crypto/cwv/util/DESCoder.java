...
import onight.tfw.outils.serialize.DesException;
...
//			encryptStr = encryptStr.replaceAll("\r\n", "").replaceAll("\n", "");
	 ...
	 * @throws DesException
	 */
	public static byte[] desEncode(byte[] content,String desKey) throws DesException{
		try {
			// 加密
			Cipher cipher = Cipher.getInstance("DES/ECB/PKCS5Padding");
			cipher.init(Cipher.ENCRYPT_MODE, genSecretKey(desKey));
	 ...
	 * @throws DesException
	 */
	public static byte[] desDecode(byte[] encBytes,String desKey) throws DesException{
		try {
			// 解密
			Cipher cipher = Cipher.getInstance("DES/ECB/PKCS5Padding");
			cipher.init(Cipher.DECRYPT_MODE, genSecretKey(desKey));
	...
	public static void main(String[] args) throws DesException {
		
		String strText = "nfebfewkjb ebwew";
		String sKey = "黄金时代里开始了打开三分大赛房间看电视看";
		byte[] kk = desEncode(strText.getBytes(),sKey);
		System.out.println("密文长度："+kk.length);
		byte[] k = desDecode(kk,sKey);
