...
import org.brewchain.mcore.api.BlockSubscriber;
import org.brewchain.mcore.api.ICryptoHandler;
import org.brewchain.mcore.bean.BlockMessage;
import org.brewchain.mcore.bean.BlockMessage.BlockMessageStatusEnum;
...
import org.brewchain.mcore.datasource.BlockChainDataAccess;
...
import org.brewchain.mcore.exception.BlockRollbackNotAllowException;
...
import org.brewchain.mcore.service.BlockMessageBuffer;
...
public class ChainHandler implements ActorService {
	...
	public synchronized void startBlockChain(String vId, String vName) {
		try {
			// TODO vAddress 不是hex
			while(!chainConfig.isKeyChainReady()) {
				Thread.sleep(1000);
			}
			...
			log.info(String.format(
					"node init   ===>   stable block: height[{%-10s}] org.brewchain.mcore.crypto.hash[{%s}]",
					...
					for (BlockInfo block : blocks) {
						...
						if (log.isDebugEnabled()) {
							log.debug(String.format(
									"node init   ===>   connect block. height[{%-10s}] org.brewchain.mcore.crypto.hash[{%s}]",
			...
			log.debug(String.format(
					"node init   ===>   max block org.brewchain.mcore.crypto.hash[%s] height[%-10s] stateroot[{}]",
	...
	private void loadGenesisDb(String path) throws Exception {
		try (InputStream genesisJsonStream = new FileInputStream(new File(path))) {
			...
			oGenesisBlock.getBody().getAccountsMap().forEach((key, value) -> {
				...
				log.info("add default v=" + oAccount);
				...
				this.stateTrie.put(oAccount.getAddress().toByteArray(), oAccount.getAddress(),
						oAccount.build().toByteArray());
			});
			...
			this.stateTrie.put(chainConfig.getConfigAddress().toByteArray(), chainConfig.getConfigAddress(),
	...
	public BlockInfo getBlockByHash(byte[] hash) {
		try {
			...
			log.error("根据hash获取区块异常 org.brewchain.mcore.crypto.hash[{}]", crypto.bytesToHexStr(hash), e);
		}
		return null;
	}
	...
	public synchronized BlockMessageMark addBlock(BlockInfo block, boolean overrided) throws Exception {
		...
		if (block.getHeader().getHeight() < maxConnectHeight) {
			// check current
			BlockInfo dbBlockInfo = getBlockByHash(parentHash);
			if (dbBlockInfo != null
					&& dbBlockInfo.getHeader().getStateRoot().equals(block.getHeader().getStateRoot())) {
				bm.setStatus(BlockMessageStatusEnum.CONNECT);
				if (maxConnectHeight < block.getHeader().getHeight()) {
					maxConnectHeight = block.getHeader().getHeight();
					maxConnectBlock = block;
		...
		if (done) {
			log.error("done with existing block :" + block.getHeader().getHeight());
		} else {
		...
		if (hash == null || hash.length == 0) {
			...
			log.error("blockheight:" + block.getHeader().getHeight() + ";addBlock() end--maxReceiveHeight:"
					+ maxReceiveHeight + ";maxConnectHeight:" + maxConnectHeight + ";maxStableHeight:" + maxStableHeight
					+ ";lastConnectHeight:" + lastConnectHeight + ",org.brewchain.mcore.crypto.hash="
	...
	public synchronized BlockMessageMark tryConnectBlock(BlockInfo block) throws Exception {
		...
		if (height < 1) {
			...
			log.debug("connect block=" + block.getHeader().getHeight() + " org.brewchain.mcore.crypto.hash="
			...
			if (blockMessageBuffer.containsHash(hash)) {
				...
				throw new BlockNotFoundInBufferException(String.format(
						"error on connect block. not found target block in block buffer. height[%-10s] org.brewchain.mcore.crypto.hash[%s]",
	...
	public synchronized BlockMessageMark tryStableBlock(BlockInfo block) throws Exception {
		...
		if (block.getHeader().getHeight() <= maxStableHeight + 1) {
			...
			log.debug("stable block " + block.getHeader().getHeight() + " hash="
			...
			log.debug("cache stable block " + block.getHeader().getHeight() + " hash="
					+ crypto.bytesToHexStr(block.getHeader().getHash().toByteArray()) + ",maxStableHeight="
					+ maxStableHeight + ",maxConnectHeight=" + maxConnectHeight);
			oBlockMessageMark.setMark(BlockMessageMarkEnum.CACHE);
		}
