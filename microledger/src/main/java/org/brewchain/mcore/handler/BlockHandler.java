...
import org.brewchain.mcore.bean.BlockMessage;
import org.brewchain.mcore.bean.BlockMessageMark;
import org.brewchain.mcore.bean.BlockMessageMark.BlockMessageMarkEnum;
import org.brewchain.mcore.bean.BlockSyncMessage;
import org.brewchain.mcore.bean.BlockSyncMessage.BlockSyncCodeEnum;
import org.brewchain.mcore.bean.TransactionExecutorSeparator;
import org.brewchain.mcore.bean.TransactionInfoWrapper;
import org.brewchain.mcore.bean.TransactionInfoWrapper.PreDefineRunner;
...
import org.brewchain.mcore.exception.BlockNotFoundInBufferException;
import org.brewchain.mcore.model.Account.AccountInfo;
import org.brewchain.mcore.model.Block.BlockBody;
import org.brewchain.mcore.model.Block.BlockHeader;
import org.brewchain.mcore.model.Block.BlockInfo;
import org.brewchain.mcore.model.Block.BlockMiner;
import org.brewchain.mcore.model.GenesisBlockOuterClass.GenesisBlock;
...
import org.brewchain.mcore.trie.NullRootNodeException;
import org.brewchain.mcore.trie.StateTrieNode;
...
public class BlockHandler implements ActorService {
	...
	int BLK_NUM_SPLIT = prop.get("org.brewchain.mcore.backend.block.split.num", 60);
			...
			Runnable subRunner = new Runnable() {
				public void run() {
					try {
						...
						// asignhash[0] = (byte) ((oBlockInfo.getHeader().getHeight() / BLK_NUM_SPLIT) %
						// 256);
						...
						if (addMark.getMark() == BlockMessageMarkEnum.APPLY) {
							...
							log.error(
									"new block  ===>   org.brewchain.mcore.crypto.hash[{}->{}] height[{}] stateroot=[{}],txcount[{}] status[{}]",
						...
						log.error("new block error。org.brewchain.mcore.crypto.hash[{}] height[{}] txcount[{}]",
	...
	public BlockSyncMessage syncBlock(BlockInfo.Builder block, boolean fastSync, int level) {
		...
		log.error(String.format(
				"start sync block   ===>   height[%-10s] org.brewchain.mcore.crypto.hash[%s] txcount[%-5s] pHash[%s] miner[%s]]",
		...
		try {
			...
			byte[] originhash = mcore.crypto.sha256(blockContent);
			// byte asignhash[] = new byte[originhash.length + 1];
			// asignhash[0] = (byte) ((applyBlock.getHeader().getHeight() / BLK_NUM_SPLIT) %
			// 256);
			// System.arraycopy(originhash, 0, asignhash, 1, originhash.length);
			...
			if (!MCoreConfig.FAST_APPLY && applyBlock.getHeader().getHeight() != 0
				...
				log.error(String.format(
						"sync block   ===>   invalid block org.brewchain.mcore.crypto.hash. height[%-10s] origin org.brewchain.mcore.crypto.hash[{%s}] current org.brewchain.mcore.crypto.hash[{%s}] ",
				...
				while (bmm.getMark() != BlockMessageMarkEnum.DONE) {
					...
					case DROP:
						...
						// org.brewchain.mcore.crypto.hash[%s]
						// 状态[%s]",
						// block.getHeader().getHeight(),
						// mcore.crypto.bytesToHexStr(block.getHeader().getHash().toByteArray()),
						// bmm.getMark()));
					...
					case CONNECTED:
						...
						log.error("height=" + block.getHeader().getHeight() + " " + bmm.getMark());
						//
						// if(block.getHeader().getHeight()==mcore.chainHandler.getMaxConnectHeight())
					...
					case APPLY:
						...
						if (parentBlock == null) {
							throw new BlockNotFoundInBufferException(
									"cannot find block with org.brewchain.mcore.crypto.hash " + mcore.crypto
											.bytesToHexStr(block.getHeader().getParentHash().toByteArray()));
						}

						if (fastSync && block.getBody().getTxsCount() > 0) {
							// mcore.transactionHandler.saveBlockTransaction(block.getBody().getTxsList());
							// for (TransactionInfo tx : block.getBody().getTxsList()) {
							// mcore.transactionHandler.syncTransactionBatch(block.getBody().getTxsList(),
							// false,
							// BigInteger.ZERO);
							// mcore.dispatcher.executeNow(fp_clean, new Runnable() {
							// @Override
							// public void run() {
							// mcore.transactionHandler.syncTxCount(block.getBody().getTxsList());
							// }
							// });
							// }
						}
						...
						if (processMessage.getSyncTxHash() != null && processMessage.getSyncTxHash().size() > 0) {
							...
							if (MCoreConfig.FAST_APPLY) {
								...
								log.error(
										"resave block  ===>   org.brewchain.mcore.crypto.hash[{}->{}] height[{}] stateroot=[{}],txcount[{}] status[{}]",
								...
								if (!block.getHeader().getStateRoot().equals(applyBlock.getHeader().getStateRoot())) {
		...
		log.error(String.format(
				"end sync block   ===>   height[%-10s] org.brewchain.mcore.crypto.hash[%s] stateroot[%s] needtx[%s] needblock[%s] currentblock[%s]",
				block.getHeader().getHeight(), mcore.crypto.bytesToHexStr(block.getHeader().getHash().toByteArray()),
	...
	private BlockSyncMessage processBlock(BlockInfo.Builder currentBlock, BlockInfo parentBlock,
		...
		if (currentBlock.getHeader().getHeight() >= 1
			...
			try {
				...
				this.mcore.stateTrie.setRoot(tempRootHash);
			} catch (NullRootNodeException e) {
				if (currentBlock.getHeader().getHeight() > 1) {
					log.error("roll back failed  org.brewchain.mcore.crypto.hash not found:height="
		...
		if (createdtxs != null && createdtxs.size() != 0) {
			...
			// try {
			// mcore.transactionHandler.removeWaitingBlockTx(txhashBytes[i], rmtime);//
			// 如果缺少hash就被扔掉了
			// } catch (Exception e) {
			// e.printStackTrace();
			// }
...
//		rewards.add(gasAccumulate.get().multiply(configAccount.getGasPrice()));
...
//		accounts.get(currentBlock.getMiner().getAddress()).addAndGet(rewards);
		...
		int accountcc = mcore.accountHandler.batchPutAccounts(currentBlock.getHeader().getHeight(), accounts);
...
//		currentBlock.getMinerBuilder().setReward(ByteString.copyFrom(BytesHelper.bigIntegerToBytes(rewards)));
		...
		long rmtime = System.currentTimeMillis();
		for (byte[] key : txkeys) {
			mcore.transactionHandler.removeWaitingBlockTx(key, rmtime);
		}
				...
				try {
					...
					try {
						...
						for (TransactionInfoWrapper itx : txs) {
							...
							// mcore.transactionHandler.removeWaitingBlockTx(itx.get);
						}
						// chainHandler.getBlockChainDA().saveBlockToFile(saveBlock.build());
						mcore.transactionHandler.batchSaveTransaction(keys, values);
					} catch (Exception e1) {
					...
					BlockHandler.this.applyPostRunner = null;
				}
				log.error("applyPostRunner.finished:" + applyPostRunner);
			}
		};
		...
		log.error("end exec block " + currentBlock.getHeader().getHeight() + " txs="
				...
				+ mcore.crypto.bytesToHexStr(tempRootHash) + " actmod=" + accountcc + ",createtx=" + createtxsize
				+ ",act=" + mcore.getChainConfig().getConfigAccount().getBalance());
	...
	public void checkParrallAndNonce(TransactionInfoWrapper[] txs,
		...
		for (TransactionInfoWrapper tx : txs) {
			...
			// }
			AccountInfoWrapper act = accounts.get(addr);
			if (seq_nonce == null) {
				if (act != null) {
					seq_nonce = new RunIndexAndNonce(new AtomicInteger(act.getNonce()), true);
				} else {
